...
public class SavingsAccount extends Account {
    ...
    private boolean isInterestApplied = false;
    ...
    public AccountOperationResponse withdraw(double amount) {
        ...
        if (isBeforeMaturity) {
            double penalty = amount * GLOBAL_EARLY_WITHDRAW_PENALTY_RATE;
            finalAmount += penalty;
        } else {
            if (!isInterestApplied) {
                interest = this.getBalance() * interestRate;
        ...
        if (finalAmount > availableBalance) {
            if (isBeforeMaturity) {
                return new AccountOperationResponse(false, "Yetersiz bakiye. Erken çekim nedeniyle %"+ (GLOBAL_EARLY_WITHDRAW_PENALTY_RATE * 100) + " ceza uygulanır. Gerekli toplam tutar: " + finalAmount);
            } else {
        ...
        return new AccountOperationResponse(true, isBeforeMaturity ? "Para çekme işlemi vade dolmadan yapıldığı için " + (GLOBAL_EARLY_WITHDRAW_PENALTY_RATE * 100) + "% kesinti ile uygulandı." : "Para çekme işlemi gerçekleşti. Vade sonu gerçekleştiği için faiz miktarı hesabınıza yatırıldı.");
    }
